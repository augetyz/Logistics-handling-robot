...
void doing_Task(void const *argument)
{
    ...
    static uint8_t Task_select = Start, color_sign = 0,IDR_date_error=3; // 执行调度参数
...
//    uint8_t IDR_X_flag = 0; // IDR_X坐标标志位
    ...
    for (;;)
    {
        ...
        if (xSemaphoreTake(Servo_Sem_Handle, 1) == pdTRUE)
                    servo_sign = 1;

        if (xQueueReceive(color_Queue, &color_date, 10) == pdTRUE)
        {
            V831_1_status = 1;
        ...
        case target_location:
            if(IDR_date_sign==1)
            {
                if ( abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > IDR_date_error )
                {
                    ...
                    IDR_speed_use = IDR_speed_dert * 3 + IDR_speed_sum2 * 0.06;
                ...
                else if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > IDR_date_error)
                {
                    ...
                    IDR_speed_use = IDR_speed_dert * 3 + IDR_speed_sum1 * 0.06;
                    IDR_speed_use = IDR_speed_use > 1000 ? 1000 : (IDR_speed_use < -1000 ? -1000 : IDR_speed_use);
