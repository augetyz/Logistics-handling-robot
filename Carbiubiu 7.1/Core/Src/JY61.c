...
uint32_t abcd=0;
float Wit_HWT101_date_deal(uint8_t* date)
{
    float angle = 0;
    uint8_t i = 0;
    int16_t angle_date = 0;
    static float angle_last=0;
    for (i = 0;i < 33;i++)
    {
        if (date[i] == 0X55&&date[i + 1] == 0X53)
        {           
            if (date[i + 2]==0&&date[i + 3]==0&&date[i + 4]==0&&date[i + 5]==0)
            {
                abcd++;
                
                angle_date = (short)((short)date[i + 7] << 8 | date[i + 6]);
                angle = (float)angle_date / 32768 * 180;
                if(abs((int)(angle_last-angle))>20)
                {
                    return 400;
                }
                else
