...
void doing_Task(void const* argument)
{
    ...
    for (;;)
    {
        ...
        case To_QR_Code:                                                                  // 前进到二维码区域
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 600) == 1) // 前进 440 mm到达二维码扫描区
            {
                speed_CTRL(0, 0, 0, 0);   // 刹车
        ...
        case At_Put_down_1_2: // 在粗加工区 放置物料,色环
            ...
            case 2:
                if (Camera_date_status == 1)
                {
                    ...
                    MaxArm_Motio = CR_Arm_adjust(color_date, color_circle_find_motion);
            ...
            case 3:
                vTaskSuspendAll();   
                MaxArm_Motio = MaxArm_landing(-75, color_date.date[1], MaxArm_Motio);
                ...
                osDelay(MaxArm_Motio.time + 70);
                vTaskSuspendAll();
                MaxArm_Motio = MaxArm_landing(110, -240, MaxArm_Motio);
                MaxArm_Motio.time = 1000;
                xTaskResumeAll(); 
        ...
        case End:
            ...
            servo_all_move(servo_change_personal(biubiu, fancy, servo_motion[1]));
            osDelay(1000);
            start_status = 0;
            vTaskResume(myTask_ledHandle); // LED闪烁任务启动
