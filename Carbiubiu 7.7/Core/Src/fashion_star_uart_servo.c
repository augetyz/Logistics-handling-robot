...
// 查询舵机的角度
uint16_t calcIntervalMs_fast(uint8_t servoId,float curAngle , float nextAngle, float speed){

	float dAngle =  (nextAngle > curAngle) ? (nextAngle - curAngle) : (curAngle - nextAngle);
	// 计算所需的时间
	return (uint16_t)((dAngle / speed) * 1000.0f);
}



// 查询舵机的角度
uint16_t calcIntervalMs(Usart_DataTypeDef *usart, uint8_t servoId, float nextAngle, float speed){
	float curAngle;
	// 读取一下舵机的角度
	FSUS_QueryServoAngle(usart, servoId, &curAngle);
	// 计算角度误差
	float dAngle =  (nextAngle > curAngle) ? (nextAngle - curAngle) : (curAngle - nextAngle);
	// 计算所需的时间
	return (uint16_t)((dAngle / speed) * 1000.0f);
...
void waitUntilServoIDLE(Usart_DataTypeDef *usart, uint8_t servoId, float nextAngle, uint16_t maxtime){
	float curAngle;
	while(1){
		// 读取一下舵机的角度
		FSUS_QueryServoAngle(usart, servoId, &curAngle);
...
// void waitUntilALLServoIDLE(Usart_DataTypeDef *usart, servo_status *servo)
// {
// 	uint8_t i;
// 	for(i=1; i<5; i++){
// 		printf("servo %d\r\n", i);
// 		waitUntilServoIDLE(usart, i, servo->value_goal[i-1]);
// 	}
// }
