...
void doing_Task(void const *argument)
{
    ...
    for (;;)
    {
        xQueueReceive(angle_Queue, &imu_date, 10);        /* 等待时间 一直等 */
        xQueueReceive(distance_Queue, &distance_now, 10); /* 等待时间 一直等 */
        ...
        {
        case Start:
            if (crosswise_angle_distance(0, imu_date, distance_now, -270) == 1) // 离开出发点左移28cm
            {
                speed_CTRL(0, 0, 0, 0);
        ...
        case To_QR_Code:
            if (advance_angle_distance(0, imu_date, distance_now, 520) == 1) // 到达二维码扫描区
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
        ...
        case At_QR_Code: // 扫描二维码
            ...
            break;
        case QR_Code_turn: // 转向90°
            if (direction_Set(turn_angle, imu_date) == 1)
            {
                speed_CTRL(0, 0, 0, 0);
                turn_sign++;
                osDelay(100);
        ...
        case To_RMA:
            if (advance_angle_distance(turn_angle, imu_date, distance_now, 465) == 1)
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
                Task_select = At_RMA;
