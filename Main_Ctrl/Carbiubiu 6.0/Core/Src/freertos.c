...
void servo_Task(void const *argument)
{
    ...
    for (;;)
    {
        if(HAL_UART_Receive(&huart4,Rx_buffer,2,0)==HAL_OK)
        {
...
//    vTaskSuspend(myTask_debugHandle); // 挂起调试任务
    ...
    for (;;)
    {
        ...
        {
        case Start:                                                                          // 开始向左走
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -1200) == 1) // 离开出发点向左 170 mm
            {
...
//                MotorMoveToSend(MotorC,10000);
                xTaskResumeAll();
            }
            break;

        case To_QR_Code:                                                                  // 前进到二维码区域
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 400) == 1) // 前进 440 mm到达二维码扫描区
            {
                speed_CTRL(0, 0, 0, 0);   // 刹车
...
//                    Camera_die(); // 关机吧
            ...
            break;
        case To_RMA:                                                                      // 前进到达原料区
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 640) == 1) // 前进 850 mm
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
                Task_select = At_RMA1;
            ...
            break;
        case At_RMA1: 
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now,100) == 1)
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
...
//                    printf("拿起蓝色\n");
        ...
        case Do_capture_2: // 第二次在原料区定位
            if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 6 || abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 6)
            {
                vTaskSuspendAll();
