...
#define IDR_color_put_y 1102

#define IDR_color_put2_x 1747 // 暂存区定位
...
void target(imu imu_date, IDR_date IDR_date_use, IDR_date IDR_date_goal, float angle_standard); // 7.20删
...
void speed_Task(void const *argument)
{
    ...
    for (;;)
    {
        if (xSemaphoreTake(Distance_Sem_Handle, 0) == pdTRUE) // 行驶距离缓存清零命令
        {
            for (i = 0; i < 4; i++)
            {
                distance.date[i] = 0;
...
void doing_Task(void const *argument)
{
    ...
    static IDR_date IDR_date_use_last = {0, 0};
    int IDR_speed_use = 0;
    ...
    //    int8_t speed_temp = 0; // 速度临时变量
    //    uint8_t IDR_Y_flag = 0; // IDR_Y坐标标志位
    //    uint8_t IDR_X_flag = 0; // IDR_X坐标标志位
    ...
    for (;;)
    {
        if (xQueueReceive(angle_Queue, &imu_date, 0) == pdTRUE)
        {
            angle_num++;
            if (angle_num > 25)
            {
        ...
        xQueueReceive(distance_Queue, &distance_now, 0); /* 等待时间 10 */

        if (xQueueReceive(IDR_date_Queue, &IDR_date_use, 0) == pdTRUE)
        {
            if (IDR_date_use.IDR_X - IDR_date_use_last.IDR_X > 5000)
            {
                IDR_date_use.IDR_X = IDR_date_use_last.IDR_X;
            }
            if (IDR_date_use.IDR_Y - IDR_date_use_last.IDR_Y > 5000)
            {
            ...
            // printf("%d %d\n", IDR_date_use.IDR_X, IDR_date_use.IDR_Y);
            IDR_num++;
            if (IDR_num > 3)
            {
                IDR_num = 0;
        ...
        {
        case Start:                                                                          // 开始向左走
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -120) == 1) // 离开出发点向左 120 mm
            {

                speed_CTRL(0, 0, 0, 0);                        // 刹车
                Task_select = Task_verify = Start_Calibration; // 跳转任务
        ...
        case Start_Calibration:                               // 第一次方向校准
            if (direction_Set(angle_standard, imu_date) == 1) // 方向校准
            {
                ...
                V831_QuickMark_do(); // 发送指令到树莓派执行二维码扫描
                xTaskResumeAll();
            }
            break;

        case To_QR_Code:                                                                  // 前进到二维码区域
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 450) == 1) // 前进 450 mm到达二维码扫描区
            {
                speed_CTRL(0, 0, 0, 0);   // 刹车
                ...
                V831_1_status = 0;               // 摄像头识别标志位归零（之前发送的数据无效，重新读取）
        ...
        case At_QR_Code:            // 扫描二维码
            if (V831_1_status == 0) // 如果摄像头标志位未触发
            {
                V831_QuickMark_do(); // 串口指令使能摄像头
                osDelay(200);
            }
            else
            {
        ...
        case Do_capture:                                                                                                  // 原料区激光定位
            if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 10 || abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 10) // 判断激光定位准确否
            {
                vTaskSuspendAll();
        ...
        case At_Put_down_1: // 到达粗加工区，进行激光校准
            if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 2 || abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 2)
            {
                vTaskSuspendAll();
        ...
        case Back_Take_thing_4:
            // if (advance_angle_distance(angle_standard, imu_date, distance_now, -120) == 1) //
            // {
        ...
        case target_location:
            ...
            if ((int16_t)(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) < -2 || (int16_t)(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 2)
            {
                ...
                IDR_speed_use = IDR_speed_dert * 1 + IDR_speed_sum2 * 0.03;
            ...
            else if ((int16_t)(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) < -2 || (int16_t)(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 2)
            {
                ...
                IDR_speed_use = IDR_speed_dert * 1 + IDR_speed_sum1 * 0.03;
                IDR_speed_use = IDR_speed_use > 1000 ? 1000 : (IDR_speed_use < -1000 ? -1000 : IDR_speed_use);
