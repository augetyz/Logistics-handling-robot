...
#define color_circle_x_RM 94   // 面向暂存区， 前- 后 +
#define color_circle_y_RM 2  // 面向粗加工区，暂存区为-，原料区为+
//四级复习： 粗加工  rough machining

#define color_circle_x_TX 94   // 面向暂存区， 
#define color_circle_y_TX 2  // 面向粗加工区，暂存区为-，原料区为+
...
//                color_date_last.date[1]=color_date.date[1];
        ...
        case Start:// 开始向左走
            ...
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -125) == 1) // 离开出发点向左 170 mm
            {
        ...
        case At_QR_Code:                 // 扫描二维码
            ...
            break;
        case To_RMA:                                                                      // 前进到达原料区
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 825) == 1) // 前进 850 mm
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
                Task_select = At_RMA1;
    ...
    //                    printf("%d,%d\n", color[color_sign], color_date.date[1]);
        ...
        case To_Turn_1: // 直线前进
            Uart5_LCD_show_string("To_Turn_1");
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 108) == 1) // 转向1
            {
...
//                    printf("拿起蓝色\n");
        ...
        case To_Turn_2:                                                                      // 左行前往暂存区
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -515) == 1) //
            {
                vTaskSuspendAll();
        ...
        case At_Turn_2:                                                                    // 直线前进到达暂存区
            if (advance_angle_distance(angle_standard, imu_date, distance_now, -612) == 1) //
            {
                vTaskSuspendAll();
        ...
        case Back_Take_thing_2:                            //右行前往原料区
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, 1140) == 1) //
            {
        ...
        case Back_Take_thing_3:                            //向后走一段
            if (advance_angle_distance(angle_standard, imu_date, distance_now, -170) == 1) //
            {
        ...
        case Stacking: // 码垛
            ...
            break;
        case Back_home_1:
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -750) == 1) //
            {
