...
void doing_Task(void const *argument)
{
    ...
    for (;;)
    {
        ...
        {
        case Start:
            if (advance_angle_distance(0, imu_date, distance_now, 1350) == 1) // 离开出发点前进28cm  1350
            {
                speed_CTRL(0, 0, 0, 0);
        ...
        case To_QR_Code:
            if (crosswise_angle_distance(0, imu_date, distance_now, 2700) == 1) // 到达二维码扫描区
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
        ...
        case At_QR_Code: // 扫描二维码
            ...
            break;
        case To_RMA:
            if (advance_angle_distance(0, imu_date, distance_now, 2475) == 1)
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
                Task_select = At_RMA1;
        ...
        case To_Turn_1:
            if (advance_angle_distance(0, imu_date, distance_now, 610 * 5) == 1) // 转向1
            {
        ...
        case To_Put_down_1:
            ...
            if (crosswise_angle_distance(angle_adjust, imu_date, distance_now, -540 * 5) == 1) // 放下1
            {
