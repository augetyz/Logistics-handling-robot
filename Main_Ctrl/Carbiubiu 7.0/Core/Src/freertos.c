...
//                color_date_last.date[1]=color_date.date[1];
        ...
        if ((GPIOC->IDR & (1 << 0)) == 0  )
        {
            ...
            if ((GPIOC->IDR & (1 << 0)) == 0 )
            {
                ...
                // /*do something*/
                start_status = 1;
                Task_select = Task_verify = To_QR_Code;        
            }
        }
        ...
        case Start:// 开始向左走
            ...
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -3000) == 1) // 离开出发点向左 115 mm
            {
...
//                Task_select = Task_verify = To_QR_Code; // 跳转任务
        ...
        case To_QR_Code:                                                                  // 前进到二维码区域
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 300) == 1) // 前进 440 mm到达二维码扫描区
            {
                speed_CTRL(0, 0, 0, 0);   // 刹车
