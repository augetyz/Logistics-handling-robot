...
#define IDR_color_grab_x 1775 // 原料区定位
#define IDR_color_grab_y 2060

#define IDR_color_put_x 1946 // 粗加工区定位
// #define IDR_color_put_y 1175
#define IDR_color_put_y 1174

#define IDR_color_put2_x 1805 // 暂存区定位
#define IDR_color_put2_y 1171

#define IDR_color_home_x 135 // 启停区定位
#define IDR_color_home_y 130
/* USER CODE END PD */
...
void doing_Task(void const *argument)
{
    ...
    for (;;)
    {
        if (xQueueReceive(angle_Queue, &imu_date, 10) == pdTRUE)
        {
            angle_num++;
            if (angle_num > 25)
            {
        ...
        if (xQueueReceive(IDR_date_Queue, &IDR_date_use, 10) == pdTRUE)
        {
            IDR_num++;
            if (IDR_num > 3)
            {
        ...
        case Do_capture:                                                                                                // 原料区激光定位
            if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 2 || abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 2) // 判断激光定位准确否
            {
                vTaskSuspendAll();
        ...
        case To_Put_down_1:                                                                  // 左行达到粗加工区
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -620) == 1) // 放下1
            {
        ...
        case To_Turn_2:                                                                      // 左行前往暂存区
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -520) == 1) //
            {
        ...
        case At_Turn_2:                                                                    // 直线前进到达暂存区
            if (advance_angle_distance(angle_standard, imu_date, distance_now, -570) == 1) //
            {
        ...
        case Back_Take_thing_2:
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, 605) == 1) //
            {
        ...
        case Stacking: // 码垛
            ...
            case 15:
                ...
                else
                {
                    ...
                    osDelay(servo_motion[30].time + 200);
                }
                break;
            case 16:
                if (servo_sign == 1)
                ...
                else
                {
                    ...
                    osDelay(servo_motion[33].time + 200);
                }
                break;
            case 22:
                if (servo_sign == 1)
            ...
            case 26:
                ...
                else
                {
                    ...
                    osDelay(servo_motion[32].time + 200);
                }
                break;
            case 27:
                if (servo_sign == 1)
                ...
                else
                {
                    ...
                    osDelay(servo_motion[35].time + 200);
                }
                break;
            case 33:
                if (servo_sign == 1)
            ...
            case 37:
                ...
                else
                {
                    ...
                    osDelay(servo_motion[31].time + 200);
                }
                break;
            case 38:
                if (servo_sign == 1)
                ...
                else
                {
                    ...
                    osDelay(servo_motion[34].time + 200);
                }
                break;
        ...
        case target_location:
            if (abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 2)
            {
                ...
                crosswise_angle(angle_standard, imu_date, IDR_speed_use);
            }
            else if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 2)
            {
                ...
                IDR_speed_use = IDR_speed_use > 1000 ? 1000 : (IDR_speed_use < -1000 ? -1000 : IDR_speed_use);
                advance_angle(angle_standard, imu_date, IDR_speed_use);
            }
            else
            {
                speed_CTRL(0, 0, 0, 0);
