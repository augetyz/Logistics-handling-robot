...
// #define JY61
#define turn_angle 0f   // 转向角度

#define UNUSED_VARIABLE(X) ((void)(X)) // 宏定义，用于消除编译器警告
#define UNUSED_PARAMETER(X) UNUSED_VARIABLE(X)  // 宏定义，用于消除编译器警告

#define IDR_color_grab_x 1108 // 用于物料区x的参数
#define IDR_color_grab_y 1801 // 用于物料区y的参数

#define IDR_color_put_x 1978  // 用于放料区x的参数
#define IDR_color_put_y 1130  // 用于放料区y的参数
...
extern servo_status servo_motion[]; // 伺服电机结构体数组，用于控制伺服电机
...
uint8_t usart_3_date[25] = {0}; // 串口3数据缓冲区，采用DMA+串口方式接收
uint8_t usart_6_date[22] = {0}; // 串口6数据缓冲区，采用DMA+串口方式接收
 
uint16_t modetime = 100; // 用于控制伺服电机的运行时间，单位ms
...
uint8_t Task_verify = Start; // 任务验证标志位，用于验证任务是否执行完成
/* USER CODE END PM */
...
SemaphoreHandle_t Servo_Sem_Handle = NULL; // 信号量初始化，用于作为伺服电机运行完成后的回响，开启SERVO_TASK函数的执行。
    ...
    for (;;)
    {
        ...
        if ((GPIOC->IDR & (1 << 1)) == 0) // 检测k2按键是否按下        
        {
            ...
            if ((GPIOE->IDR & (1 << 1)) == 0)
            {
                ...
                servo_all_move(servo_motion[1]); // 机械臂复位
            }
        }
        osDelay(1);
    }
...
void IMU_Task(void const *argument)
{
    ...
    /* Infinite loop */
    int i; // 用于循环
    imu imu_car; // 用于存储IMU数据
    OS_status = 1;  // 用于标记IMU数据是否正常
    ...
    for (;;)
    {
...
#else
        for (i = 0; i < IMU_speed; i++) // 一次性处理IMU_speed个数据
        {
            decode_succ = ch_serial_input(&raw, IMU_date[i]); // 解码
            if (decode_succ == 1)                            // 解码成功
                break;
        }
        ...
        if (decode_succ) // 解码成功
        {
            for (i = 0; i < 3; i++) // 读取欧拉角
            {
                imu_car.IMU[i] = raw.imu[0].eul[i]; // 读取欧拉角
            }
        }
#endif
        HAL_UART_Receive_DMA(&huart2, IMU_date, IMU_speed); // 重新开启DMA接收
        taskEXIT_CRITICAL();
        xQueueSend(IMU_Queue, &imu_car, 0); // 发送数据
        xQueueSend(angle_Queue, &imu_car, 0); // 发送数据
    }
...
void deubg_Task(void const *argument)
{
    ...
    /* Infinite loop */
    imu imu_car; // 用于存储IMU数据
    biu speed_now; // 用于存储当前速度
    biu speed_goal;  // 用于存储目标速度
    float angle[5]; // 用于存储舵机角度
    uint16_t i = 0; // 用于循环
    for (;;)
    {

        xQueueReceive(IMU_Queue, &imu_car, 10); // 接收IMU数据
        xQueueReceive(debug_Queue, &speed_goal, 10); // 接收目标速度 
        xQueuePeek(Speed_Queue, &speed_now, 10); // 接收当前速度

        for (i = 0; i < 5; i++)
        {
            FSUS_QueryServoAngle(servoUsart, i + 1, &angle[i]); // 读取舵机角度
            osDelay(10);
        }
        ...
        sprintf((char *)debug_date, "%.1f,%.1f,%.1f,%.1f,%.1f,%.2f\n", angle[0], angle[1], angle[2], angle[3], angle[4], imu_car.IMU[2]);   //  上传舵机角度
        HAL_UART_Transmit_DMA(&huart1, debug_date, strlen((char *)debug_date)); // 串口一DMA发送
        taskEXIT_CRITICAL();
...
void usart_Task(void const *argument)
{
    ...
    int i = 0, num, sign = 0; // 用于循环
    int16_t x = 0;            // 用于存储x坐标
    int16_t y = 0;            // 用于存储y坐标
    int16_t wide = 0;         // 用于存储宽度
    int16_t height = 0;       // 用于存储高度

    char *f = (char *)&x; // 用于存储x坐标
    char *h = (char *)&y;   // 用于存储y坐标
    char *u = (char *)&wide;    // 用于存储宽度
    char *q = (char *)&height;  // 用于存储高度
    biu_int_16 color_date;       // 用于存储颜色
    IDR_date IDR_date_get;      // 用于存储IDR数据
    ...
    for (;;)
    {
        ...
        if (xSemaphoreTake(usart3_RX_Sem_Handle, 10) == pdTRUE) /* 等待时间 */
        {
            ...
            for (i = 0; i < 8; i++)
            {
                if (usart_3_date[i] == 0X2C && usart_3_date[i + 10] == 0X5B) // 判断是否为目标数据
                {
                    num = i;
                    if (usart_3_date[num + 1] == 0X12) // 判断是否为红色
                        sign = 1;
                    else if (usart_3_date[num + 1] == 0X21) // 判断是否为蓝色
                        sign = 2;
                    else
                        sign = 0;   // 不是目标数据
                    break;
            ...
            if (sign == 1)   // 红色    
            {
                ...
                for (i = 0; i < 2; i++)
                {
                    *(f + i) = *(usart_3_date + num + i + 2); // 读取x坐标
                }
                //          i=*(f);*(f)=*(f+1);*(f+1)=i;

                for (i = 0; i < 2; i++)
                {
                    *(h + i) = *(usart_3_date + num + i + 4);   // 读取y坐标
                }
                for (i = 0; i < 2; i++)
                {
                    *(u + i) = *(usart_3_date + num + i + 6);  // 读取宽度
                }
                for (i = 0; i < 2; i++)
                {
                    *(q + i) = *(usart_3_date + num + i + 8);   // 读取高度
                }
                //          i=*(h);*(h)=*(h+1);*(h+1)=i;
                if ((x != 520 && y != 520) && (y != 9)) // 判断是否为有效数据
                {
                    color_date.date[0] = x;  // 上传数据
                    color_date.date[1] = y;  // 上传数据
                    color_date.date[2] = 11; // 上传数据
                    color_date.date[3] = height; // 上传数据
                    xQueueSend(color_Queue, &color_date, 0); // 上传数据
                }
                else
                {
                    // error
            ...
            else if (sign == 2) // 蓝色
            {
                ...
                if (x != 0)
                {
                    if ((x & 3 << 0) != ((x & 3 << 2) >> 2) && (x & 3 << 0) != ((x & 3 << 4) >> 4) && (x & 3 << 2) >> 2 != ((x & 3 << 4) >> 4))
                    {
                        ...
                        color_date.date[3] = x; // 上传数据
                        color_date.date[2] = 22; // 上传数据
                        xQueueSend(color_Queue, &color_date, 0);    // 上传数据
                    }
                    else
                    {
        ...
        // USART6
        if (xSemaphoreTake(usart6_RX_Sem_Handle, 10) == pdTRUE) /* 等待时间 */ // 串口6接收数据
        {
            vTaskSuspendAll();
            i = 0; 
...
void pid_Task(void const *argument)
{
    ...
    for (;;)
    {
        ...
        pid_do(speed_goal, speed_now); /* pid计算 */
...
void doing_Task(void const *argument)
{
    ...
    biu_int_16 color_date;  // 颜色数据结构体
    IDR_date IDR_date_use, IDR_date_goal; // 电感数据结构体

    static int back_sign = 0; // 退回标志位
    static int16_t distance_use = 0; // 距离使用变量

    static uint8_t Task_select = Start, color_sign = 0; // 执行调度参数

    static float angle_adjust = 0; // 角度调整变量

    int8_t color[6] = {1, 2, 3, 0, 0, 0}; // 颜色顺序存储

    uint8_t Arm_do_order = 0,  // 机械臂执行顺序
            servo_sign,       // 机械臂执行标志位
            V831_1_status = 0, // V831状态
            center_color_sign = 0, // 中心颜色标志位
            color_sign_now,       // 当前颜色标志位
            target_location_sign = 0; 


    vTaskSuspend(myTask_debugHandle); // 挂起调试任务
    UNUSED_VARIABLE(distance_use); // 未使用变量
    
    Task_verify = Task_select = Start; // 初始化任务选择参数
    speed_CTRL(0, 0, 0, 0); // 初始化速度控制
    for (;;)
    {
        ...
        //      imu_date.IMU[2]=0;
        if (xSemaphoreTake(Servo_Sem_Handle, 0) == pdTRUE) // 机械臂信号量
            servo_sign = 1; 
        ...
        case Start_Calibration: // 出发点校准
            if (direction_Set(0, imu_date) == 1)
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
                Task_select = To_QR_Code; // 到达二维码扫描区
                Task_verify = To_QR_Code; // 到达二维码扫描区
                //                V831_QuickMark_do();
                xTaskResumeAll();
            }
            break;

        case To_QR_Code: // 前往二维码扫描区
            if (crosswise_angle_distance(0, imu_date, distance_now, 520) == 1) // 到达二维码扫描区
            {
                speed_CTRL(0, 0, 0, 0); // 停止
                vTaskSuspendAll();
                // Task_select = At_QR_Code;
                // Task_verify = At_QR_Code;
                Task_select = To_RMA; // 到达RMA区
                Task_verify = To_RMA; // 到达RMA区
                V831_1_status = 0;
                xTaskResumeAll();
            }
        ...
        case At_QR_Code: // 扫描二维码
            ...
            else
            {
                ...
                else
                {
                    ...
                    V831_die();
                    vTaskSuspendAll();
                    Task_select = QR_Code_turn; // 转向二维码扫描区
                    Task_verify = QR_Code_turn; // 转向二维码扫描区
                    V831_1_status = 0;
            ...
            break;
        case To_RMA: // 前往RMA区
            if (advance_angle_distance(0, imu_date, distance_now, 465) == 1)
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
                Task_select = Do_capture; // 抓取
                Task_verify = Do_capture; // 抓取 
            ...
            break;
        case Do_capture: // 抓取
            if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 5 && abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 5) // 判断是否到达目标点
            {
                vTaskSuspendAll();
                back_sign = Do_capture; // 抓取
                Task_select = target_location; // 目标点
                Task_verify = target_location; // 目标点
                xTaskResumeAll();
            }
            else
            {
        ...
        case Take_color_thing:
        {
            ...
            case 1: // 抓取第一步
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        osDelay(servo_motion[4].time + 500);
                    }
                }
                break;
            case 2:
            ...
            case 3: // 抓取第二步
                ...
                else
                {
                    ...
                    osDelay(servo_motion[5].time + 500);
                }
                break;
            case 4: // 抓取第三步
                if (servo_sign == 1)
                ...
                else
                {
                    ...
                    osDelay(servo_motion[6].time + 500);
                }
                break;
            case 5: // 抓取第四步
                if (servo_sign == 1)
                ...
                else
                {
                    ...
                    osDelay(servo_motion[7].time + 500);
                }
                break;
            case 6:
        ...
        case At_Put_down_1: // 到达半加工区，进行激光校准
            if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 5 && abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 5)
            {
                vTaskSuspendAll();
        ...
        case target_location:
            if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 5)
            {
                if (IDR_date_goal.IDR_X - IDR_date_use.IDR_X > 0)
                    advance_angle(0, imu_date, 50);
                else
                    advance_angle(0, imu_date, -50);
            }
            else if (abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 5)
            {
                if (IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y > 0)
