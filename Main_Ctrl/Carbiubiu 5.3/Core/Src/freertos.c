...
#define color_circle_x 80
#define color_circle_y -10
...
//                color_date_last.date[1]=color_date.date[1];
        ...
        case Start:// 开始向左走
            ...
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -120) == 1) // 离开出发点向左 170 mm
            {
                ...
                servo_all_move(servo_motion[2]); // 机械臂调整
...
//                servo_all_move(servo_motion[3]);
    ...
    //                    printf("%d,%d\n", color[color_sign], color_date.date[1]);
        ...
        case To_Turn_1: // 直线前进
            Uart5_LCD_show_string("To_Turn_1");
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 100) == 1) // 转向1
            {
        ...
        case To_Put_down_1:                                                                  // 左行达到粗加工区
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -670) == 1) // 放下1
            {
...
//                    printf("拿起蓝色\n");
        ...
        case At_Turn_2:                                                                    // 直线前进到达暂存区
            if (advance_angle_distance(angle_standard, imu_date, distance_now, -620) == 1) //
            {
        ...
        case To_Put_down_2: // 在暂存区调整车身角度 
            if (direction_Set_biu(angle_standard, imu_date) == 1)
            {
                speed_CTRL(0, 0, 0, 0);
                osDelay(500);
                xQueueReceive(angle_Queue, &imu_date, 10);
                xQueueReceive(angle_Queue, &imu_date, 10);
                if (direction_Set(angle_standard, imu_date) == 1)
                {
                    vTaskSuspendAll();
        ...
        case Back_Take_thing_2:                            //右行前往原料区
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, 1130) == 1) //
            {
        ...
        case Back_Take_thing_3:                            //向后走一段
            if (advance_angle_distance(angle_standard, imu_date, distance_now, -135) == 1) //
            {
        ...
        case Stacking: // 码垛
            ...
            break;
        case Back_home_1:
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -790) == 1) //
            {
            ...
            break;
        case Back_home_2:
            if (advance_angle_distance(angle_standard, imu_date, distance_now, -1270) == 1) //
            {
        ...
        case Color_cicle_location:
            if(Camera_date_status==1)
            {
                if (abs(color_date.date[0] - color_circle_x) > 2)
                {
                    color_speed_dert = color_date.date[0] - color_circle_x;
                    color_speed_dert = abs(color_speed_dert) > 5 ? color_speed_dert : color_speed_dert * 5;
                    color_speed_sum2 += color_speed_dert;
                    color_speed_sum2 = color_speed_sum2 > 2000 ? 2000 : (color_speed_sum2 < -2000 ? -2000 : color_speed_sum2);
                    color_speed_use = color_speed_dert * Direction_KP + color_speed_sum2 * Direction_KI;
                    color_speed_use = color_speed_use > 1000 ? 1000 : (color_speed_use < -1000 ? -1000 : color_speed_use);
                    advance_angle(imu_date.IMU[2], imu_date, color_speed_use);
                }
                else if (abs(color_date.date[1] - color_circle_y) > 2)
                {
                    color_speed_sum2 = 0;
                    color_speed_dert = color_circle_y-color_date.date[1] ;
                    color_speed_dert = abs(color_speed_dert) > 5 ? color_speed_dert : color_speed_dert * 5;
                    color_speed_sum1 += color_speed_dert;
                    color_speed_sum1 = color_speed_sum1 > 2000 ? 2000 : (color_speed_sum1 < -2000 ? -2000 : color_speed_sum1);
                    color_speed_use = color_speed_dert * Direction_KP + color_speed_sum1 * Direction_KI;
                    color_speed_use = color_speed_use > 1000 ? 1000 : (color_speed_use < -1000 ? -1000 : color_speed_use);
                    crosswise_angle(imu_date.IMU[2], imu_date, color_speed_use);
                }
                else
                {
                    speed_CTRL(0, 0, 0, 0);
                    color_speed_sum1 = color_speed_sum2 = 0;
                    osDelay(200);
                    if (target_location_sign < 6)
                    {
