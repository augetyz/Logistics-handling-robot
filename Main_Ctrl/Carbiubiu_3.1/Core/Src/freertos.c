...
void doing_Task(void const *argument)
{
    ...
    for (;;)
    {
        ...
        {
        case Start:
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -170) == 1) // 离开出发点向左 170 mm
            {
        ...
        case To_QR_Code:
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 450) == 1) // 前进 450 mm到达二维码扫描区
            {
                ...
                Uart5_LCD_send_task(1);          // LCD串口屏指令发送
                xTaskResumeAll();
            }
        ...
        case To_Turn_1:
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 170) == 1) // 转向1
            {
        ...
        case To_Put_down_1:
            ...
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -570) == 1) // 放下1
            {
        ...
        case At_Put_down_1_3: // 把放下的物料拿起来
            ...
            break;
        case To_Turn_2:
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -540) == 1) //
            {
            ...
            break;
        case At_Turn_2:
            if (advance_angle_distance(angle_standard, imu_date, distance_now, -540) == 1) //
            {
        ...
        case Back_Take_thing_2:
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, 1040) == 1) //
            {
        ...
        case Back_Take_thing_3:
            if (advance_angle_distance(angle_standard, imu_date, distance_now, -100) == 1) //
            {
                ...
                IDR_date_goal.IDR_X = IDR_color_grab_x;
                IDR_date_goal.IDR_Y = IDR_color_grab_y;
                Task_select = Task_verify = Do_capture_2;
                xTaskResumeAll();
            }
            break;
        case Do_capture_2:
