...
uint8_t advance_angle_distance(float angle, imu imu_date, biu distance_now, int distance_goal)
{
    ...
    distance_dert_sum = distance_dert_sum > 5000 ? 5000 : (distance_dert_sum > -5000 ? distance_dert_sum : -5000);
    distance_dert_last = distance_dert;

    if (distance_dert < 20 && distance_dert > -20)
    {
        speed_CTRL(0, 0, 0, 0);
        return 1;
    }
...
uint8_t crosswise_angle_distance_low(float angle, imu imu_date, biu distance_now, int distance_goal)
{
    int k = 305, d = 0;
    distance_goal = distance_goal * k + d;
    int speed = 0, goal_out;
    ...
    speed_goal = kp4 * distance_dert + kd4 * (distance_dert - distance_dert_last) + ki4 * distance_dert_sum;
    speed_goal = speed_goal > speed_best ? speed_best : (speed_goal > -speed_best ? speed_goal : -speed_best);
    distance_dert_sum = distance_dert_sum > 3000 ? 3000 : (distance_dert_sum > -3000 ? distance_dert_sum : -3000);
    distance_dert_last = distance_dert;
    ...
    else
    {
        ...
        if (angle_dert < -1 || angle_dert > 1)
        {
            ...
            goal_out = 0;
        speed = -goal_out;
        speed_goal = speed_goal > 10 ? 10 : (speed_goal < -10 ? -10 : speed_goal);
        goal_out = goal_out > 60 ? 60 : (goal_out < -60 ? -60 : goal_out);
        speed_CTRL(speed_goal + speed, -speed_goal - speed, -speed_goal + speed, speed_goal - speed);
        return 0;
    }
    return 0;
