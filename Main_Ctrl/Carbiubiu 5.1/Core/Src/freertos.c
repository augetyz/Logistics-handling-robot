...
#define IDR_color_put_y  1100 //1100
...
void doing_Task(void const *argument)
{
    ...
    uint8_t Arm_do_order = 0, // 表示需要执行动作组的编号
            ...
            back_home_error = 0;// 第二次去拿取物料时，如果中断则开启此标志位
    int16_t color_cicle_x=0,
            color_cicle_y=0;
    ...
    for (;;)
    {
        ...
        case Do_capture:                                                                                                                          // 原料区激光定位
            if (abs(IDR_date_goal.IDR_X - IDR_date_use.IDR_X) > 6 || abs(IDR_date_goal.IDR_Y - IDR_date_use.IDR_Y) > 6) // 判断激光定位准确否
            {
                vTaskSuspendAll();
