...
#define color_circle_y_RM -45
//四级复习： 粗加工  rough machining

#define color_circle_x_TX 7   
#define color_circle_y_TX -45

#define color_Stacking_x -45  
#define color_Stacking_y 40
...
#define TS_RMA_right            1535
#define TS_RMA_Back             -430
#define RMA_Rough_go_2          450
#define RMA_Rough_left_2        -930
#define Rough_TS_left_2         -645
#define Rough_TS_Back_2         -825
#define TS_Start_back           -1710
#define TS_Start_left           -1065

//色环逆解调整
#define Red_Put_H               -85     //红色色环放置高度
#define Red_Get_W               -440    //红色色环回收距离(最小为Ychange-10)
#define Green_Put_H             -57     //绿色色环放置高度
#define Blue_Put_H              -75     //蓝色色环放置高度
#define Blue_Get_W              -440    //蓝色色环回收距离

#define Red_Rotary_table        97
#define Green_Rotary_table      -30
#define Blue_Rotary_table       -142
...
void doing_Task(void const* argument)
{
    ...
    static uint8_t Task_select = Start,
        ...
        use_num = 0;        // 执行调度参数
    servo_status servo_time_use = { 0 }, MaxArm_Motion = { 0 }, MaxArm_Motion_check = { 0 }, color_circle_find_motion = { 0 };
    ...
    for (;;)
    {
        ...
        case To_QR_Code:                                                                  // 前进到二维码区域
            if (advance_angle_distance(angle_standard, imu_date, distance_now, Start_CR_Go) == 1) // 前进 440 mm到达二维码扫描区
            {
                speed_CTRL(0, 0, 0, 0);   // 刹车
        ...
        case At_QR_Code:                 // 扫描二维码
            ...
            break;
        case To_RMA:                                                                      // 前进到达原料区
            if (Directional_move_distance(angle_standard, imu_date, distance_now, CR_RMA_go) == 1) // 前进 845 mm  , 当原料区距离修改时，第一转角距离也需要修改
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
                Task_select = At_RMA1;
        ...
        case To_Turn_1: // 直线前进
            Uart5_LCD_show_string("To_Turn_1");
            if (advance_angle_distance(angle_standard, imu_date, distance_now, RMA_Rough_go) == 1) // 向前前往转向1
            {
        ...
        case To_Put_down_1:                                                                  // 左行达到粗加工区
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, RMA_Rough_left) == 1) // 放下1
            {
        ...
        case S_To_Turn_1: // 直线前进
            Uart5_LCD_show_string("To_Turn_1");
            if (advance_angle_distance(angle_standard, imu_date, distance_now, RMA_Rough_go_2) == 1) // 向前前往转向1
            {
                Uart5_LCD_show_string("To_Put_down_1");
        ...
        case S_To_Put_down_1:                                                                  // 左行达到粗加工区
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, RMA_Rough_left_2) == 1) // 放下1
            {
        ...
        case Color_cicle_location:  // 判断颜色圈位置
            if (Camera_date_status == 1)  // 如果相机数据状态为1
            {
                ...
                else if (abs(color_date.date[1] - color_circle_y) > 6)  // 否则，如果颜色数据的第二个元素与颜色圈的y坐标的差的绝对值大于2
                {
