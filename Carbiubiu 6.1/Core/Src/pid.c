...
void pid_do(biu speed_goal, biu speed_now)
{
    ...
    int16_t speed_dert[4],speed_best=2980;
    uint8_t i = 0;
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        speed_sum[i] = speed_sum[i] > 40000 ? 40000 : (speed_sum[i] < -40000 ? -40000 : speed_sum[i]); // 积分限幅
...
uint8_t advance_angle_distance(float angle, imu imu_date, biu distance_now, int distance_goal)
{
    ...
    int speed = 0, goal_out;
    int speed_goal, speed_best = 2500;
    float angle_dert;
    ...
    xTaskResumeAll();
    if (distance_dert < 75 && distance_dert > -75)
    {
        speed_CTRL(0, 0, 0, 0);
        angle_dert_sum = 0;
        distance_dert_sum = 0;
    ...
    else
    {
        ...
        goal_out = angle_dert * kp3 + angle_dert_sum * ki3 + (angle_dert - angle_dert_last) * kd3;
        goal_out = goal_out > speed_best / 5 ? speed_best / 5 : (goal_out > -speed_best / 5 ? goal_out : -speed_best / 5);
