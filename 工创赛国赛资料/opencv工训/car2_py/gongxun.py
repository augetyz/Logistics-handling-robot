...
def sehuan():
    greenLower = np.array([41,51,84])
    greenUpper = np.array([92,255,255])
    mybuffer = 64  #初始化追踪点的列表
    pts = deque(maxlen=mybuffer)
    task3 = ser.read()
    while (task3 != b'D' and task3 != b'A' and task3 != b'B'):
        ...
        if len(cnts_list)>0:
            ...
            if len(cnts_list_1) > 0:       
                ...
                #epsilon = 0.04 * cv2.arcLength(c_green, True) #多边形拟合的距离参数，下一个函数要用到。原理见代码后链接
