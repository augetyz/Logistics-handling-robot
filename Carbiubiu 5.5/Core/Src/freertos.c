...
#define color_circle_x 70   // 面向暂存区， 
#define color_circle_y -13  // 面向粗加工区，暂存区为-，原料区为+
...
//                    
                // /*do something*/
                start_status = 1;
                Task_select = Task_verify = At_RMA1;
            }
        }
...
//                servo_all_move(servo_motion[2]); // 机械臂动作调整
        ...
        case At_QR_Code:                 // 扫描二维码
            ...
            break;
        case To_RMA:                                                                      // 前进到达原料区
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 840) == 1) // 前进 850 mm
            {
                speed_CTRL(0, 0, 0, 0);
                vTaskSuspendAll();
                Task_select = At_RMA1;
    ...
    //                    printf("%d,%d\n", color[color_sign], color_date.date[1]);
        ...
        case To_Turn_1: // 直线前进
            Uart5_LCD_show_string("To_Turn_1");
            if (advance_angle_distance(angle_standard, imu_date, distance_now, 95) == 1) // 转向1
            {
...
//                    printf("拿起蓝色\n");
        ...
        case To_Turn_2:                                                                      // 左行前往暂存区
            if (crosswise_angle_distance(angle_standard, imu_date, distance_now, -508) == 1) //
            {
